...
use crate::{db::Engine, errors::Result, index::IndexerIterator, options::IteratorOptions};

// 迭代器接口
pub struct Iterator<'a> {
    // 索引迭代器
    index_iter: Arc<RwLock<Box<dyn IndexerIterator>>>,
    engine: &'a Engine,
}
...
impl Engine {
    ...
    {
        let iter = self.iter(Default::default());
